function [] = make_IASI_hrCRIS_sno(req_date,cris_res);
%
% function [] = make_IASI_hrCRIS_sno(par1,par2)
%
% produce files of SNO using CRIS from CCAST processing at UMBC.ASL
%
% INPUT PARAMETERS:
%   par1. req_date: string 'YYYY/MM/DD' to process
%   par2. cris_res: string {'high','low'}
%
...
if(strcmp(cris_res,'LOW'))
  savDir = '/asl/s1/chepplew/data/sno/iasi_cris/LR/';
...
% ************    Get CRIS_hr granule files and load geolocation  *****************
...
% ************  Load up CRIS SNO Obs data ************
  ...
  tcatrk = [];    tcxtrk = []; tcfov = [];
  tctim  = [];     tclat = []; tclon = [];  tcszn = [];  tcsaz = []; tclnfr = [];
  for fn = 1:numel(crLst)
    load(strcat(crDir,crLst(fn).name));
    errFlg = reshape(L1a_err,[],1);                     % (30 x nscan) 1: bad, 0: good.
    junk   = reshape(geo.FORTime(~L1a_err),[],1);       % <- (30 x 60) per granule. nu-secs since 1958
...
% subset on center track (FORs 15 and 16)
  ...
  % subset onto center track b4 using upos index.
      ...
      junk = csaz(indx);          tcsaz  = [tcsaz; junk(cSams-ps+1)];    clear junk;
    end
    ps = pe + 1;
...
% ****************   Load up IASI SNO Obs data ************
  ...
  titim  = [];  tilat = []; tilon = [];  tiszn = [];   tisaz = []; tilnfr = [];
  ...
  for fn = 1:numel(iaLst);                        % fails at fn=423 IASI_xxx_1C_M02_20130827213858Z_20130827214154Z.gz corrupt file
   ...
   iasaz    = reshape(s.Satellite_Zenith',[],1);
...
% subset on center track (FORs 15 and 16) to update counters
  ...
  % subset onto center track b4 using upos index.
      ...
      junk = iasaz(indx);          tisaz  = [tisaz; junk(iSams-ps+1)];    clear junk;
    end
    ps = pe + 1;
...
% Saving data ****************
  ...
  clear itime ilat ilon isolzen isatzen ifov ri;
  clear ctime clat clon csolzen csatzen cfov rc tdiff dist;
  itime = titim;  ilat = tilat; ilon = tilon; isolzen = tiszn; ifov = tifov; isatzen = tisaz;
  ctime = tctim;  clat = tclat; clon = tclon; csolzen = tcszn; cfov = tcfov; csatzen = tcsaz;
  dist  = pre.uPhi'; tdiff = pre.utdiff;
  ri    = tirad; 
  rc    = [tcrLW; tcrMW; tcrSW]';  fc = [vLW; vMW; vSW];
  savVars = {'strYr','strMn','strDy','dist','tdiff','upos','maxDtim','maxDphi', ...
             'itime','ilat','ilon','isolzen','isatzen','ifov','ri',...
             'ctime','clat','clon','csolzen','csatzen','cfov','rc','fc'};
  if(strcmp(cris_res,'HIGH')) 
    savFN = strcat(savDir, strYr,'/sno_iasi_cris_asl_',strYr,strMn,strDy,'.mat');
  elseif(strcmp(cris_res,'LOW'))
    savFN = strcat(savDir, strYr,'/sno_iasi_cris_asl_',strYr,strMn,strDy,'.mat');  
  end
